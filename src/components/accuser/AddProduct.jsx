...
const AddProduct = ({setOpenAdd}) => {
    ...
    const handlerAddProduct = async (e) => {
        ...
        if(!imageProduct){
            return alert("Please select Product's Image");
        }
        ...
        try {
            ...
            console.log("Error!");
            alert("Adding new Product failed, please try again!")
    ...
    return (
        ...
        <form onSubmit={handlerAddProduct}>
            ...
            <div className="form-group form-image product">
                <img src={previewImageProduct || "https://res.cloudinary.com/daklbeheh/image/upload/v1667333658/defaultVeggie_ox0ggt.png"} alt="" />
                <label htmlFor="image_upload_product"> <FontAwesomeIcon icon={faPlusCircle}/></label>
                <input type="file" hidden id="image_upload_product" accept="image/png, image.jpeg" onChange={validateImageProduct}/>
            </div>

            <div className="form-btn">
                <button type="submit"><FontAwesomeIcon icon={faPlusCircle}/> {uploadingImageProduct ? "Saving..." : "Add Product"}</button>
            </div>
        </form>
    </div>
    )
